...
import { ENGLISH_GUESS_WORDS } from "./english-guess-words";
...
const ACTIVE_GAME_KEY = "brief:active-game";
...
function getSolutionList(mode: Mode) {
  return mode === "daily" ? DAILY_SOLUTIONS : BRAND_SOLUTIONS;
}

function getAnswer(mode: Mode, briefNumber: number, gameNumber: number) {
  const list = getSolutionList(mode);
  return list[(briefNumber - 1 + gameNumber) % list.length];
}

function activeGameKey(mode: Mode, dateKey: string) {
  return `${ACTIVE_GAME_KEY}:${mode}:${dateKey}`;
}

function legacyProgressKey(mode: Mode, dateKey: string) {
  return `brief:progress:${mode}:${dateKey}`;
}

function progressKey(mode: Mode, dateKey: string, gameNumber: number) {
  return `${legacyProgressKey(mode, dateKey)}:${gameNumber}`;
}
...
function continuesStreak(previousDateKey: string | null, currentDateKey: string) {
  if (!previousDateKey) {
    return false;
  }
  ...
  return (
    ...
    daysBetween(previousDateKey, currentDateKey) === 1
  );
}
...
export default function Home() {
  ...
  const answer = useMemo(
    () => getAnswer(mode, gameDay.briefNumber, gameNumber),
    [gameDay.briefNumber, gameNumber, mode],
  );
  ...
  const resetBoard = useCallback(() => {
    ...
    setMessage("Fresh brief. New answer.");
    setShareMessage("");
    setShowStats(false);
    window.localStorage.setItem(
      activeGameKey(mode, gameDay.dateKey),
      String(nextGameNumber),
    );
    window.localStorage.removeItem(
      progressKey(mode, gameDay.dateKey, nextGameNumber),
    );
  }, [gameDay.dateKey, gameNumber, mode]);
  ...
  useEffect(() => {
    ...
    const storedGameNumber = Number.parseInt(
      window.localStorage.getItem(activeGameKey(mode, gameDay.dateKey)) ?? "0",
      10,
    );
    setGameNumber(
      Number.isFinite(storedGameNumber) && storedGameNumber >= 0
        ? storedGameNumber
        : 0,
    );
  }, [gameDay.dateKey, mode, mounted]);
    ...
    try {
      const raw =
        window.localStorage.getItem(
          progressKey(mode, gameDay.dateKey, gameNumber),
        ) ??
        (gameNumber === 0
          ? window.localStorage.getItem(legacyProgressKey(mode, gameDay.dateKey))
          : null);
      if (!raw) {
        setGuesses([]);
        setCurrentGuess("");
  ...
  }, [answer, gameDay.dateKey, gameNumber, mode, mounted]);
  ...
  useEffect(() => {
    ...
    setStats((currentStats) => {
      ...
      const nextCurrentStreak = won
        ? continuesStreak(current.lastPlayedDate, gameDay.dateKey)
    ...
    <main className="brief-app">
      <header className="topbar" aria-label="BRIEF navigation">
        <a className="brand-lockup" href="#game" aria-label="BRIEF home">
          <Image
            className="mccann-wordmark"
            src="/brand-assets/mccann-wordmark-white.svg"
            alt="McCann"
            width={947}
            height={226}
            priority
